...
The primary functionality is from two functions: `fisheye_fgc` which operates on a data.frame, and `sf_fisheye` which operates on an `sf` object. Operationally, both are the same, except that the elements in the object being transformed are different. 

The first two columns are the x and y coordinates to be transformed. Also specified are: `cx` and `cy`, denoting the 'center' of the fisheye transformation, `r_in` and `r_out`, which define the radius of the transformed circles, and `zoom_factor` which controls how zoomed the inner-most "focus" layer is.

As a demonstration, the below code snippet transforms a subset of hospitals in metropolitan Victoria, using data from the `mapycusmaximus` package.
...
The effect of this transformation can be seen in Figure \@ref(fig:usage-demo). Points near the specified centre are more spread in the transformed data (panel (b)) compared to in the original data (panel (a)). Here, points are coloured based on whether they are part of the context, glue, or focus sections. Points in the context and glue section do not move, while those in the focus layer are generally pushed outwards from the centre.
...
A second function, `sf_fisheye`, extends the `fisheye_fgc` function for use in spatial contexts. This function takes a spatial object, which can be multilayered, and then similar to the function on points, a center, radius and zoom factor. The resulting object is also an `sf` object, which can be used as a drop-in replacement in downstream visualisations.

This process is shown in Figure \@ref(fig:`r ifelse(knitr::is_html_output(), 'plot-fgc-html', 'plot-fgc')`). Although it is hard to tell from the figure, here the polygons are continuous, and boundaries are preserved. This means that features such as shading polygons will work as expected.
