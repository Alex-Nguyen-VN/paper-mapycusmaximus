...
filtered_locations <-  mapycusmaximus::hospital_locations |>
  filter(longitude >= 144, longitude <= 146, latitude <= -37, latitude >= -39)

hospitals_transformed <- fisheye_fgc(filtered_locations[, -1], cy = 145, cx = -37.8, r_in = 0.3, r_out = 0.5, zoom_factor = 2)

head(hospitals_transformed)
```
...
hospitals_fisheye_plot <- hospitals_transformed |>
  as_tibble() |>
  mutate(zone = attr(hospitals_transformed, "zone"), zone = factor(zone, levels = c("context", "glue", "focus")))

hospitals_fisheye <- hospitals_fisheye_plot |>
  ggplot(aes(x=y_new, y = x_new, colour = zone)) + geom_point() +
  theme_void() +
  coord_fixed() + 
...
hospitals_original + hospitals_fisheye + plot_annotation(tag_levels = list(c("(a)", '(b)'))) & theme(plot.tag.position = c(0.04, 0.97))
```
...
This process is shown in Figure \@ref(fig:sf-usage-map). Although it is hard to tell from the figure, here the polygons are continuous, and boundaries are preserved. This means that features such as shading polygons will work as expected.

```{r sf-usage-map, fig.cap="Fisheye transformation applied to the map of Victoria, Australia. The center is focussed on the Melbourne metropolitan region, cannot typically be seen in standard spatial maps."}
sf_fisheye(mapycusmaximus::vic, center = c(145, -37.8)) |>
  ggplot() +
  geom_sf(fill = "grey85", color = "white") +
  coord_sf() +
  theme_map()
```
