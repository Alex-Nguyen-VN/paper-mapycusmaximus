...
transfers_plot <- ggplot() +
  ...
  geom_sf(data = conn_fish_sub, aes(size=weight, alpha=weight, label=weight), color = "black") +
...
ggplotly(transfers_plot, width=800, height=610) |>
...
bind_w <- sf_fisheye(bind, center = metro_center, r_in = 0.2, r_out = 0.3, zoom = 4, squeeze = 0.35)
vic_w   <- bind_w |> dplyr::filter(.layer == "vic") |> dplyr::select(-.layer)
hosp_w   <- bind_w |> dplyr::filter(.layer == "hosp_pt") |> dplyr::select(-.layer)
racf_w   <- bind_w |> dplyr::filter(.layer == "racf_pt") |> dplyr::select(-.layer)
...
focus <- ggplot() +
  ...
  geom_sf(data = transfers_w, aes(size=weight, alpha=weight, label=weight), 
          color = "black") +
  geom_sf(data = hosp_w, aes(label = hosp_name), color = "red", size = 1) +
  geom_sf(data = racf_w, aes(label = racf_name), color = "blue", size = 1) +
  scale_size(range = c(0.2, 2), guide = "none") +
  scale_alpha(range = c(0.4, 1), guide = "none") + 
...
ggplotly(focus, width=800, height=610) |>
  layout(hovermode = "closest", 
  xaxis = list(autorange = TRUE), 
  yaxis = list(autorange = TRUE))
```
