...
Two functions are provided to use the package. The first, `fisheye_fgc`, takes a `data.frame` where the first two columns are the x and y coordinates to be transformed. Also specified are: `cx` and `cy`, denoting the 'center' of the fisheye transformation, `r_in` and `r_out`, which define the radius of the transformed circles, and `zoom_factor` which controls how zoomed the inner-most "focus" layer is.
