...
Figure \@ref(fig:`r ifelse(knitr::is_html_output(), 'hospitals-interactive-plot', 'hospitals-basic-plot')`) shows the geographical map of local government areas c overlaid by locations of RACFs (blue) and hospitals (red) overlaid with transfers shown as lines. The rural transfers are easy to see, but transfers in the metropolitan area cannot be seen on this map. The fisheye transformation can be used to alleviate this.
...
transfers_plot <- ggplot() +
  ...
  geom_sf(data = conn_fish_sub, color = "black", linewidth = 0.5) +
  geom_sf(data = hosp_pts, aes(label = hosp_name), color = "red", size = 1) +
  geom_sf(data = racf_pts, aes(label = racf_name), color = "blue", size = 1) +
...
```{r hospitals-basic-plot, fig.cap="Victorian map overlaid by locations of RACFs (blue) and hospitals (red), and lines indicating transfers. Transfers in the metropolitan region cannot be fully seen.", fig.width=8, fig.height=6, layout="l-body", out.width="100%", eval=knitr::is_latex_output()}
transfers_plot 
```

```{r hospitals-interactive-plot, fig.cap="Victorian map overlaid by locations of RACFs (blue) and hospitals (red), and lines indicating transfers. Transfers in the metropolitan region cannot be fully seen.", out.width="100%", fig.width = 8, fig.height=6, layout="l-body-outset", eval=knitr::is_html_output()}
...
hosp_pts_w <- pts_w |> st_drop_geometry() |> st_as_sf(coords = c("long_hosp","lat_hosp"), crs = 4326, remove = FALSE)
racf_pts_w <- pts_w |> st_drop_geometry() |> st_as_sf(coords = c("long_racf","lat_racf"), crs = 4326, remove = FALSE)
  
#vic_w <- sf_fisheye(vic, center = metro_center, r_in = 0.2, r_out = 0.3, zoom = 7, squeeze = 0.35)
#pts_w <- sf_fisheye(conn_fish_sub, center = metro_center, r_in = 0.2, r_out = 0.3, zoom = 7, squeeze = 0.35)
```

```{r fisheye-plot-prepare}
focus <- ggplot() +
  geom_sf(data = vic_w, fill = "grey85", color = "white") +
  geom_sf(data = transfers_w, color = "black", size = 0.5) +
  geom_sf(data = hosp_pts_w, aes(label = hosp_name), color = "red", size = 1) +
  geom_sf(data = racf_pts_w, aes(label = racf_name), color = "blue", size = 1) +
