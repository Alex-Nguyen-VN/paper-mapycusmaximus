...
```{r new-lga-source, eval=FALSE}
remotes::install_github("JustinCally/vicspatial")
library(vicspatial)
check_geoserver()
available_layers <- listLayers()
sf_lga_vic <- vicmap_query(layer = "open-data-platform:ad_lga_area_polygon") |> collect()
sf_lga_vic <- sf_lga_vic |>
  filter(state=="VIC")
sf_lga_vic <- sf_lga_vic |>
  filter(!str_detect(official_name, "UNINC"))

sf_lga_vic_pop <- left_join(sf_lga_vic, pop, by = c("name" = "lga_key"))
...
vic_lga2 <- vic_lga |> 
  ...
  mutate(lga_key = clean_lga(LGA_NAME))

vic_lga2_pop <- left_join(vic_lga2, pop, by = "lga_key")

ggplot() +
 geom_sf(data = vic_lga2_pop, aes(fill = pop), color = "grey85") +
  scale_fill_distiller("Population", "BuPu") +
  theme_map() +
  theme(
    legend.position = "top",
...
# https://discover.data.vic.gov.au/dataset/vicmap-lite-local-government-area-lga-polygon-1-250000-to-1-5000000
vic_lga <- read_sf("data/VMLITE/VMLITE_LGA.shp")
vic_lga <- vic_lga |>
  filter(STATE == "VIC")
```

```{r fix-vic, eval=FALSE}
...
vic_fixed <- vic |>
  ...
  filter(!str_detect(lga_key, "UNINC")) 

st_geometry(vic) <- remove_holes(st_geometry(vic))

save(vic_fixed, file="data/vic_fixed.rda")
```
...
load("data/vic_fixed.rda")
vic_pop_sf <- left_join(vic_fixed, pop, by = "lga_key")
vic_pop_merc <- st_transform(vic_pop_sf, 3857)

# Do I need this? Why isn't this done in the version of the data in the package? There are 8 UNINC areas which are not LGAs, to be removed also
# vic2 <- vic |> 
#   filter(!str_detect(LGA_NAME, "UNINC")) |>
#   mutate(lga_key = clean_lga(LGA_NAME))
# 
# vic_pop_sf <- left_join(vic2, pop, by = "lga_key")
# vic_pop_merc <- st_transform(vic_pop_sf, 3857)
# 
# # Why do we need this one?
# vic_pop <- vic_pop_merc |>
#   select(LGA_NAME, pop, geometry) |>
#   st_make_valid() |>
#   st_transform(7844)

#pal <- brewer.pal(nlevels(vic_pop$population_round), "BuPu")
...
main_map <- ggplot() +
 ...
 geom_sf(data = lga_inset, aes(fill = pop), color = "black") + # Highlight the inset area
  #scale_fill_manual(
  #  values = pal,
  ...
  #scale_fill_distiller("Pop ('00000)", palette="YlGnBu", direction = 1) +
...
inset_map <- ggplot() +
  geom_sf(data = lga_inset, aes(fill = pop), color = "white", linewidth = 0.2) +
  scale_fill_viridis_c("Pop ('00000)", direction = -1) +
...
final_map <- ggdraw() +
 ...
 draw_plot(inset_map, x = 0.5, y = 0.5, width = 0.5, height = 0.5) # Inset map position and size
...
```{r sugarbag-plot-pdf, echo=FALSE, fig.cap="Sugarbag hex tile map illustrating thematic spatial abstraction. Original LGA polygons are replaced by uniform hexagons, with fill indicating poplulation and original geography shown faintly beneath. This representation removes area bias but sacrifices precise geographic location.", eval=knitr::is_latex_output()}
hextilemap
```

```{r sugarbag-plot-html, echo=FALSE, fig.cap="Sugarbag hex tile map illustrating thematic spatial abstraction. Original LGA polygons are replaced by uniform hexagons, with fill indicating poplulation and original geography shown faintly beneath. This representation removes area bias but sacrifices precise geographic location.", eval=knitr::is_html_output()}
ggplotly(hextilemap, tooltip = "label") |> layout(
...
carto_plot <- ggplot(cartogram) +
  geom_sf(aes(fill = pop, label=lga_key), linewidth = 0.05, alpha = 0.9, color = "black") +
  scale_fill_viridis_c("Pop ('00000)", direction = -1) +
  theme_void() +
  theme(
