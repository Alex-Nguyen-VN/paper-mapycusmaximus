...
transform_grid_1 <- fisheye_fgc(test_grid, zoom_factor = 1.2)
transform_grid_2 <- fisheye_fgc(test_grid, zoom_factor = 1.7, revolution = pi/2)
...
plot_1 <- ggplot() +
  geom_point(data = transform_grid_1, aes(x = x_new, y = y_new, color = zones)) +
  geom_path(data = transform_grid_1, aes(x = x_new, y = y_new, color = zones)
  )+ scale_color_discrete() + 
  coord_fixed(ratio = 1)
plot_2 <- ggplot() +
  geom_point(data = transform_grid_2, aes(x = x_new, y = y_new, color = zones)) +
  scale_color_discrete() + 
  coord_fixed(ratio = 1)
...
d <- cube.line(p = 2, n = 8)
d_seg <- tibble(
  x = d$points[d$edges[,1],1],
  xend = d$points[d$edges[,2],1], 
  y = d$points[d$edges[,1],2],
  yend = d$points[d$edges[,2],2])
...
ggplot() +
  ...
  geom_sf(data = vic_w |> filter(LGA_NAME == "MELBOURNE"), fill = "white", linewidth = 1, color ="red") +
  geom_sf_label(data = melbourne, aes(label = "Melbourne CBD"), size = 3) +
