...
For multi-layer maps, the normal process is combine all the layers into a single sf object and apply `sf_fisheye()`, then split the result later. One minialist example for this approach is show in the code block \@ref{fig:multi-layer-example}.

```{r multi-layer-example, eval=FALSE, echo=TRUE, fig.cap="Example of multi-layer fisheye transformation.", fig.align="center"}
# Multi-layer example
