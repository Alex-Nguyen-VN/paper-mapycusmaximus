...
\section{Alternative approaches to the detail-context problem}\label{alternative-approaches-to-the-detail-context-problem}
...
\subsubsection{Design and extensibility}\label{sec:multi-layer-example}
...
For multi-layer maps, the normal process is combine all the layers into a single sf object and apply \texttt{sf\_fisheye()}, then split the result later. One minialist example for this approach is show in the code block @ref\{sec:multi-layer-example\}.
...
(\(0.55\text{--}0.70\)). A \texttt{zoom\_factor} of \(5\text{--}10\) provides visible enlargement
without overwhelming the transition.

\begin{figure}
\includegraphics[width=0.8\linewidth]{paper-mapycusmaximus_files/figure-latex/zoom-factor-1} \caption{Zoom factors for balanced metropolitan focus within a state.}\label{fig:zoom-factor}
\end{figure}

Pair this with A \texttt{squeeze\_factor} of \(0.3\text{--}0.5\), which
...
silhouette and adjacency rather than precise edge angles. Settings such as \texttt{\{squeeze\_factor\ =\ 0.25\textbackslash{}text\{\ -\ \}0.40\}} with \texttt{zoom\_factor\ =\ 5\textbackslash{}text\{\ -\ \}20\}} and either \texttt{"expand"} or
\texttt{"outward"} often work well.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{paper-mapycusmaximus_files/figure-latex/method-plot-1.pdf}}
\caption{\label{fig:method-plot}Different method for glue compression}
\end{figure}

We recommend \texttt{revolution\ =\ 0} for publication unless the swirl
...
A pattern that works well is to fix \(r_{\text{in}}, r_{\text{out}}\) and \texttt{squeeze\_factor}, and vary
...
\textbf{Choosing radii from map scale.} When stakeholders communicate distances in kilometres or miles, convert desired physical radii to unit radii using the bbox half-span. With \texttt{\{preserve\_aspect\ =\ TRUE\}}, \(r_{\text{in}} = d/s\) where \(d\) is the intended focus radius in map units (metres for metric projections) and \(s\) is the larger half-span of the bbox.

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{paper-mapycusmaximus_files/figure-latex/radii-prepare-1} 

}

\caption{Map scale for different radii}\label{fig:radii-prepare-1}
\end{figure}
\begin{figure}

{\centering \includegraphics[width=1\linewidth]{paper-mapycusmaximus_files/figure-latex/radii-prepare-2} 

}

\caption{Map scale for different radii}\label{fig:radii-prepare-2}
\end{figure}

This rule allows quick calibration: for a state with half-span 250 km, a desired 75 km focus corresponds to \(r_{\text{in}} \approx 0.30\). For per-axis scaling, choose semi-axes independently: \(r_{\text{in,x}} = d_x/s_x\), \(r_{\text{in,y}} = d_y/s_y\), noting that the current implementation interprets \(r_{\text{in}}\) as a single scalar and therefore realises an ellipse only through \texttt{preserve\_aspect\ =\ FALSE}.
...
\textbf{Publication vs.~exploration.} For exploratory notebooks and talks, small nonzero \texttt{revolution} values (\(\le 0.3\) radians) can help audiences perceive continuity across the glue.

\begin{figure}

{\centering \includegraphics{paper-mapycusmaximus_files/figure-latex/fisheye-revolution-1} 

}

\caption{Fisheye with different revolutions.}\label{fig:fisheye-revolution}
\end{figure}

For manuscripts and dashboards, prefer \texttt{revolution\ =\ 0}. Similarly, start with \texttt{"expand"} and adopt \texttt{"outward"} only when outer stability is an explicit requirement. Always annotate or at least describe the distortion in figure captions so readers do not mistake warped areas for standard
projections.
...
\caption{\label{fig:hospitals-basic-plot}Standard maps: hospital (red) and RACF (blue) points plotted over Victoria without fisheye.}
\end{figure}
...
\caption{\label{fig:fisheye-plot}Fisheye applied with a common centre and parameters keeps overlays aligned while magnifying the Melbourne LGA.}
\end{figure}
...
The France grid example uses \texttt{france\_sport.gpkg} from B. Nowak's repository (github.com/BjnNowak/lego\_map), where \texttt{value} denotes the percentage of population who are members of a sport association. We classify \texttt{value} into six bins for a gridded choropleth and overlay centroids for reference.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{paper-mapycusmaximus_files/figure-latex/france-comparison-1.pdf}}
\caption{\label{fig:france-comparison}France choropleth before (left) and after (right) fisheye: the lens improves legibility around the selected metropolitan area while preserving national context.}
\end{figure}
