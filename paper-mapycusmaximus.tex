...
\(s_x, s_y\) are bbox half-spans; with \texttt{FALSE}, independent scales are used per axis. Uniform scaling preserves circular symmetry of the focus and glue; per-axis scaling yields an ellipticalinterpretation that can be usefulcommon for long, narrow extents but should be used deliberately. Degenerate
...
\begin{figure}
\includegraphics[width=0.8\linewidth]{paper-mapycusmaximus_files/figure-latex/zoom-factor-plot-1} \caption{ZEffect of zoom factor on fisheye distortion. Two panels compare zoom factors 1.5 and 2 applied to a synthetic grid, with points coloured by zone. Higher zoom increases magnification in the focus while preserving context stability.}\label{fig:zoom-factor-plot}
\end{figure}
...
\item
  \textbf{Original (static)}: a conventional \texttt{ggplot2}/\texttt{sf} map rendered with the \emph{same bounding box and aspect ratio} as the fisheye view, enabling fair visual comparison between warped and unwarped representations.
\end{itemize}
...
\tightlist
\item
  \textbf{Zoom factor} -\textgreater{} \texttt{zoom\_factor} / \texttt{zoom} Controls radial expansion inside the focus. Higher values increase separation of dense features but amplify distortion near the focus boundary.
\item
  \textbf{Squeeze} -\textgreater{} \texttt{squeeze\_factor} / \texttt{squeeze} Controls how sharply distances compress within the glue region. Smaller values produce a pronounced ``shoulder'' near boundaries; larger values yield a smoother transition.
\end{itemize}
