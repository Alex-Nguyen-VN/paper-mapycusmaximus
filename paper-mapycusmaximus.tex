...
Two functions are provided to use the package. The first, \texttt{fisheye\_fgc}, takes a \texttt{data.frame} where the first two columns are the x and y coordinates to be transformed. Also specified are: \texttt{cx} and \texttt{cy}, denoting the `center' of the fisheye transformation, \texttt{r\_in} and \texttt{r\_out}, which define the radius of the transformed circles, and \texttt{zoom\_factor} which controls how zoomed the inner-most ``focus'' layer is.

As a demonstration, the below code snippet transforms a subset of hospitals in metropolitan Victoria, using data from the \texttt{mapycusmaximus} package.
...
A second function, \texttt{sf\_fisheye}, extends the \texttt{fisheye\_fgc} function for use in spatial contexts. This function takes a spatial object, which can be multilayered, and then similar to the function on points, a center, radius and zoom factor. The resulting object is also an \texttt{sf} object, which can be used as a drop-in replacement in downstream visualisations.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{paper-mapycusmaximus_files/figure-latex/sf-usage-map-1.pdf}}
\caption{\label{fig:sf-usage-map}Fisheye transformation applied to the map of Victoria, Australia. The center is focussed on the Melbourne metropolitan region, cannot typically be seen in standard spatial maps.}
\end{figure}
