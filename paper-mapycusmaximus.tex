...
\caption{Population of Victorian LGAs shown as a diffusion cartogram. Color (and size) indicates population. We can see that the LGAs for greater Melbourne have the highest population, and these are massively exploded.}\label{fig:plot-cart}
\end{figure}
...
The FGC fisheye lens keeps everything in one frame - roads bend smoothly, metropolitan detail enlarges, but you still see how the city sits within its state. It's a geometric \emph{warp} rather than a data-driven \emph{substitution} or panel-based \emph{separation}. This matters for applications such as examining hospital networks in Melbourne while maintaining the tsate context, exploring Census tracts in a metro core without losing county boundaries, or studying transit lines with their regional hinterland visible.

\section{Usage}\label{usage}

The primary functionality is from two functions: \texttt{fisheye\_fgc} which operates on a \texttt{data.frame} or \texttt{tibble}, or \texttt{sf} object. Operationally, both are the same, except that the elements in the object being transformed are different: The first two columns in the former, and the geographic coordinates in the latter.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{paper-mapycusmaximus_files/paper-mapycusmaximus_files/figure-latex/usage-demo-1.pdf}}
...
\caption{Fisheye transformation applied to the map of Victoria, Australia. The center is focussed on the Melbourne metropolitan region, which cannot typically be seen in standard spatial maps.}\label{fig:plot-fgc}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{paper-mapycusmaximus_files/paper-mapycusmaximus_files/figure-latex/fgc-zones-1.pdf}}
\caption{\label{fig:fgc-zones}Illustration of Focus-Glue-Context zones in a fisheye transformation. Original grid points are shown alongside their transformed positions, colored by zone, with arrows indicating displacement. Points expand in the focus, compress smoothly (inwards or outwards) in the glue, and remain fixed in the context.}
\end{figure}
...
Fundamentally, spatial objects in the \texttt{sf} package are just lists of points, with a structure of how they are layered and connected together. Thus, the \texttt{fisheye\_fgc} effectively loops over these layers, each time applying a transformation function to each layer, and then reconstructing the \texttt{sf} object based on it's original inputs. Geometries are split by type:
...
\begin{figure}
\includegraphics[width=0.8\linewidth]{paper-mapycusmaximus_files/figure-latex/zoom-factor-plot-1} \caption{Effect of zoom factor on fisheye distortion. Two panels compare zoom factors 1.5 and 2 applied to a synthetic grid, with points colored by zone. Higher zoom increases magnification in the focus while preserving context stability.}\label{fig:zoom-factor-plot}
\end{figure}
...
Together, these define the Focus-Glue-Context structure used by \texttt{fisheye\_fgc()}. Increasing \texttt{r\_out} spreads distortion more gradually; decreasing it concentrates deformation closer to the focus.
